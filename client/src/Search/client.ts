...
export const createFlightInspiration = async (flightInspiration: any) => {
    const response = await axiosWithCredentials.post(`${REMOTE_SERVER}/api/flightInspiration/`, flightInspiration);
    return response.data;
}

export const findAllSearchHistory = async () => {
    const response = await axiosWithCredentials.get(`${REMOTE_SERVER}/api/flightInspiration/`);
    return response.data;
}

export const findPostsByCity = async (city:String) => {
    const response = await axiosWithCredentials.get(`${REMOTE_SERVER}/api/posts/getPostsByLocation/${city}`);
    return response.data;
}

export const findPostsByCityAndCountry = async (city:String, country:String) => {
    const response = await axiosWithCredentials.get(`${REMOTE_SERVER}/api/posts/getPostsByLocation?city=${city}&country=${country}`);
    return response.data;
}

export const findPostsByQuery = async (query:String) => {
    const response = await axiosWithCredentials.get(`${REMOTE_SERVER}/api/posts/getPostsByQuery?query=${query}`);
    return response.data;
}